<x-client-layout>
    ...
    @can('contact_create')
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                <a class="btn btn-success" href="{{ route('account.contacts.create') }}">
                ...
                <table class=" table table-bordered table-striped table-hover datatable datatable-Contact">
                    ...
                    @foreach($contacts as $key => $contact)
                        <tr data-entry-id="{{ $contact->id }}">
                            ...
                            <td>
                                @can('contact_edit')
                                    <a class="btn btn-sm btn-primary" href="{{ route('account.contacts.edit', $contact) }}">
                                        <i class="bi bi-pencil"></i> {{ trans('global.edit') }}
                                    </a>
                                @endcan
                                @can('contact_delete')
                                    <form action="{{ route('account.contacts.destroy', $contact->uuid) }}" method="POST" class="d-inline" onsubmit="return confirm('{{ trans('global.areYouSure') }}');">
                                        @csrf
                                        @method('DELETE')
