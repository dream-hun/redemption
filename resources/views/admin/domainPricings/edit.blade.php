            ...
            <form method="POST" action="{{ route("admin.domain-pricings.update", [$domainPricing->id]) }}"
                ...
                <div class="form-group">
                    <label class="required"
                           for="register_price">{{ trans('cruds.domainPricing.fields.register_price') }}</label>
                    <input class="form-control {{ $errors->has('register_price') ? 'is-invalid' : '' }}" type="number"
                           name="register_price" id="register_price"
                           value="{{ old('register_price', $domainPricing->register_price) }}" step="1" required>
                ...
                <div class="form-group">
                    <label class="required"
                           for="transfer_price">{{ trans('cruds.domainPricing.fields.transfer_price') }}</label>
                    <input class="form-control {{ $errors->has('transfer_price') ? 'is-invalid' : '' }}" type="number"
                           name="transfer_price" id="transfer_price"
                           value="{{ old('transfer_price', $domainPricing->transfer_price) }}" step="1" required>
                ...
                <div class="form-group">
                    <label class="required"
                           for="renew_price">{{ trans('cruds.domainPricing.fields.renew_price') }}</label>
                    <input class="form-control {{ $errors->has('renew_price') ? 'is-invalid' : '' }}" type="number"
                           name="renew_price" id="renew_price"
                           value="{{ old('renew_price', $domainPricing->renew_price) }}" step="1" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('grace') ? 'is-invalid' : '' }}" type="number"
                           name="grace" id="grace" value="{{ old('grace', $domainPricing->grace) }}" step="1">
                    @if($errors->has('grace'))
                        <div class="invalid-feedback">
                            {{ $errors->first('grace') }}
                        </div>
                ...
                <div class="form-group">
                    <label class="required"
                           for="redemption_price">{{ trans('cruds.domainPricing.fields.redemption_price') }}</label>
                    <input class="form-control {{ $errors->has('redemption_price') ? 'is-invalid' : '' }}" type="text"
                           name="redemption_price" id="redemption_price"
                           value="{{ old('redemption_price', $domainPricing->redemption_price) }}" required>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status"
                        ...
                        @foreach(App\Models\DomainPricing::STATUS_SELECT as $key => $label)
                            <option
                                value="{{ $key }}" {{ old('status', $domainPricing->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
